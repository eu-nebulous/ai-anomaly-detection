...
# The original file can be found at https://github.com/netdata/netdata-pandas

__all__ = ['get_chart_list', 'get_chart', 'get_charts', 'get_data', 'get_alarm_log', 'get_allmetrics', 'get_allmetrics_async']
...
def get_chart_list(host: str = '127.0.0.1:19999', starts_with: str = None, ends_with: str = None, protocol: str = 'http',
    verify: Union[str, bool] = True, timeout: int = 10) -> List[str]:
    """Check if a `host` is not valid or accessible. If all is ok, get a list of all available charts on a `host`.
    ...
    ##### Returns:
    - **chart_list** `List[str]` A list of available charts or `None` if the `host` is not valid or accessible.
    """
    url = f"{protocol}://{host}/api/v1/charts"
    try:
        print(f"Fetching charts from {url} with timeout {timeout} seconds")
        logger.info(f"Fetching charts from {url} with timeout {timeout} seconds")
...
def get_chart_list_with_parent(parent: str = None, host: str = '127.0.0.1:19999', starts_with: str = None, ends_with: str = None, protocol: str = 'http',
    verify: Union[str, bool] = True, timeout: int = 10) -> List[str]:
    """Check if a `host` is not valid or accessible. If all is ok, get a list of all available charts on a `host`.
    ...
    - **chart_list** `List[str]` A list of available charts or `None` if the `host` is not valid or accessible.
    """
    url = f"{protocol}://{parent}/host/{host}/api/v1/charts"
    try:
        print(f"Fetching charts from {url} with timeout {timeout} seconds")
        logger.info(f"Fetching charts from {url} with timeout {timeout} seconds")
        r = requests.get(url, verify=verify, timeout=timeout)
...
async def get_chart(api_call: str, data: list, col_sep: str ='|', numeric_only: bool = True, float_size: str = 'float64',
    ...
    try:
        ...
        logger.info(f'error found on data from: {url}')
...
async def get_available_range(api_call: str):
    url, chart, host, user, pwd = api_call
    try:
        if user and pwd:
            user_pwd = (user, pwd)
        ...
        # Extract time values
        time_index = r_json['labels'].index('time')
        timestamps = [entry[time_index] for entry in r_json['data']]
        ...
        print(f'Error fetching range from: {url} with error {str(e)}')
        logger.info(f'Error fetching range from: {url} with error {str(e)}')
        return None, None
...
def get_data(hosts: list = ['london.my-netdata.io'], charts: list = ['system.cpu'], after: int = -60,
    ...
    # Verify that each host in the `hosts` list is correct.
    for host in hosts:
        #if host not in get_chart_list(host, verify=verify):
        if get_chart_list(host, verify=verify) is None:
            print(f"IMPORTANT!!! Host '{host}' is not valid or accessible.")
            logger.info(f"IMPORTANT!!! Host '{host}' is not valid or accessible.")
            raise ValueError(f"Host '{host}' is not valid or accessible.")
    ...
    try:
        ...
        print(f"Error fetching charts data: {e} - after {after} before {before}")
        logger.error(f"Error fetching charts data: {e} - after {after} before {before}")
        return None
...
def get_data_with_parent(parent: str = 'london.my-netdata.io', hosts: list = ['cdn77', 'newyork.netdata.rocks', 'sanfrancisco', 'bangalore', 'd1.firehol.org', 'toronto.netdata.rocks', 'singapore.netdata.rocks', 'frankfurt.netdata.rocks'], charts: list = ['system.cpu'], after: int = -60,
             before: int = 0, points: int = 0, col_sep: str = '|', numeric_only: bool = True,
             ffill: bool = True, diff: bool = False, timeout: int = 60, nunique_thold = None,
             std_thold: float = None, index_as_datetime: bool = False, freq: str = 'infer',
             group: str = 'average', sort_cols: bool = True, user: str = None, pwd: str = None,
             ...
             host_charts_dict: dict = None, host_prefix: bool = False, host_sep: str = ':',
             charts_regex: str = None, verify: Union[str, bool] = True, dimensions: str = '*',
             options: str = '', context: str = '', TOPIC_NAME: str = '') -> pd.DataFrame:
    """Define api calls to make and any post processing to be done.
    ...
    # Verify that each host in the `hosts` list is correct.
    for host in hosts:
        #if host not in get_chart_list_with_parent(host, verify=verify):
        if get_chart_list_with_parent(parent, host, verify=verify) is None:
            print(f"IMPORTANT!!! Host '{host}' is not valid or accessible.")
            logger.info(f"IMPORTANT!!! Host '{host}' is not valid or accessible.")
            anomaly_data = {
                "anomaly": {
            ...
            context.publishers[TOPIC_NAME].send(anomaly_data)
            raise ValueError(f"Host '{host}' is not valid or accessible.")
    ...
    if host_charts_dict:
        ...
        charts_regex = re.compile(charts_regex)
        host_charts = [(host, chart) for host in hosts for chart in list(filter(charts_regex.match, get_chart_list_with_parent(host, verify=verify)))]
    elif charts == ['all']:
        host_charts = [(host, chart) for host in hosts for chart in get_chart_list_with_parent(host, verify=verify)]
    else:
        host_charts = [(host, chart) for host in hosts for chart in charts]
    ...
    api_calls = [
        (
            f'{protocol}://{parent}/host/{host_chart[0]}/api/v1/data?chart={host_chart[1]}&after={after}&before={before}&points={points}&format=json&group={group}&dimensions={dimensions}&options={options}',
            host_chart[1],
            host_chart[0],
            user,
            pwd
...
def get_parentchildren(host='london.my-netdata.io', protocol: str = 'http'):
    r = requests.get(f"{protocol}://{host}/api/v1/info")
    parentchildren = r.json()['mirrored_hosts']
